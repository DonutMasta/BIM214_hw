...
public class App {
        public static void main(String[] args) {
                ...
                do {
                        double qN = c.UnitOrderedPerReplenishmentNew(orderingCost, annualDemand, holdingCost, unitCost,
                                        nR);
                        fR = c.Frformula(qN, holdingCost, annualDemand, unitCost);
                        double rN = c.Rformula(leadTimeDemand, leadTimeStandartDeviation, fR, sc);
                ...
                double averageAnnualHoldingCost = holdingCost
                                * (Math.round(q0) / 2 + Math.round(rSecond) - leadTimeDemand);
                ...
                System.out.println(
                                "Average annual holding , setup and penalty costs are: "
                                                + df.format(averageAnnualHoldingCost) + ", "
                                                + df.format(averageAnnualOrderingCost) + ", "
                                                + df.format(averageAnnualPenaltyCost));

                double averageTimeBetweenOrders = (Math.round(q0) / annualDemand) * 12;

                System.out.println(
                                "Average time between the placement of orders: " + df.format(averageTimeBetweenOrders)
                                                + " months");

                double poc = 1 - fR; // The proportion of order cycles in which no stock out occurs

                System.out.println(
                                "The proportion of order cycles in which no stock out occurs: " + df.format(poc * 100)
                                                + "%");

                double pod = nR / q0; // The proportion of demands that are not met

                System.out.println("The proportion of demands that are not met: " + df1.format(pod));
